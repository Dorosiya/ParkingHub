  ...
  <head>
    ...
    <style>
      ...
      .main-content {
        ...
        min-height: calc(
          100vh - 150px
        ); /* 푸터와 네비게이션 바 높이를 제외한 최소 높이 */
        padding-bottom: 20px; /* 푸터와의 간격 추가 */
      }

      #map {
        width: 100%;
        flex: 1;
        min-height: 600px; /* 기존 500px에서 600px로 늘림 */
        height: calc(100vh - 250px); /* 화면 높이를 고려한 계산 */
        border-radius: 8px;
      ...
      .footer {
        ...
        padding: 20px 0; /* 기존 30px에서 20px로 축소 */
        margin-top: auto; /* 항상 아래에 위치하도록 설정 */
      }
    ...
    <script>
      ...
      function searchNearbyParking(lat, lng) {
        ...
        listContainer.innerHTML =
          '<div class="p-3 text-center text-muted">주변 주차장을 검색 중입니다...</div>';
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response && response.length > 0) {
                ...
                return $.ajax({
                  ...
                  .then(function (details) {
                    var item = {
                      ...
                      isPublic:
                        parking.prkPlaceNm &&
                        parking.prkPlaceNm.includes("공영"),
                      isFree: false,
                      fee: "정보 없음",
                      total: 0,
                      available: 0,
                    ...
                    if (details && details.realtime) {
                      ...
                      item.available =
                        details.realtime.pkfcAvailableParkingLotsTotal || 0;
                    }

                    if (details && details.operation) {
                      item.fee = details.operation.feeInfo || "정보 없음";
                      item.isFree =
                        details.operation.feeInfo &&
              ...
              listContainer.innerHTML =
                '<div class="p-3 text-center text-muted">주변 주차장 정보가 없습니다.</div>';
            }
          },
          error: function (xhr, status, error) {
            ...
            listContainer.innerHTML +=
              '<div class="alert alert-warning mt-2">API 연결 실패로 임시 데이터를 표시합니다.</div>';
          },
        });
      }
      ...
      function updateParkingList() {
        ...
        if (filteredParking.length === 0) {
          listContainer.innerHTML =
            '<div class="p-3 text-center text-muted">조건에 맞는 주차장이 없습니다.</div>';
          return;
        }
        ...
        filteredParking.forEach(function (parking) {
          ...
          kakao.maps.event.addListener(marker, "click", function () {
            var content =
              ...
              '<a href="/parking/' +
              parking.id +
              '" class="btn btn-sm btn-primary" style="font-size: 12px;">상세보기</a>' +
              "</div>" +
              "</div>";
          ...
          var content =
            ...
            '<a href="/parking/' +
            parking.id +
            '" class="btn btn-sm btn-primary" style="font-size: 11px;">상세보기</a>' +
            "</div>" +
            "</div>";
